...
public class EncuentroVinicius : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (miAnimator != null && TieneParametro("estaCorriendo"))
        {
    ...
    bool TieneParametro(string nombreParam)
    {
        if (miAnimator == null) return false;
        foreach (AnimatorControllerParameter param in miAnimator.parameters)
        {
            if (param.name == nombreParam) return true;
        }
        return false;
    }
}